<?php
	// Ejemplo de operadores y expresiones PHP - htts://www.dinapyme.com
	// Operadores de comparación. !=
	// En este ejemplo dos pruebas con el operador != (diferente o no igual).
	
	$numero = 10;  // variable para los ejemplos
	
	// Título
	echo "<center><h1>Operadores de Comparación: != (diferente)</h1></center>";
	
	// mostrar si el contenido de la variable $número es diferente de 10.
	echo "Comprobar si \$numero es diferente a 10: ";
	echo ($numero != 10);  // Muestra 1 porque es diferente.
	echo "<br />";
	
	// mostrar si el contenido de la variable $numero es diferente de 5
	echo "Comprobar si \$numero es diferente a 5: ";
	echo ($numero != 5);  // No muestra 1 porque no es diferente
	
?>